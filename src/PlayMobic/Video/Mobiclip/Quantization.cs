...
internal class Quantization
{
    ...
    private readonly int[] table4x4;
    private readonly int[] table8x8;
    ...
    public Quantization(int qp)
    {
        ...
        table4x4 = new int[4 * 4];
        for (int i = 0; i < table4x4.Length; i++) {
            table4x4[i] = Block4x4Table[tableIdx][i] << scale;
        }

        table8x8 = new int[8 * 8];
        for (int i = 0; i < table8x8.Length; i++) {
            table8x8[i] = Block8x8Table[tableIdx][i] << (scale - 2);
        }
    }
    ...
    public void Quantize(int[] block)
    {
        ...
        int[] table = (block.Length == 16) ? table4x4 : table8x8;
